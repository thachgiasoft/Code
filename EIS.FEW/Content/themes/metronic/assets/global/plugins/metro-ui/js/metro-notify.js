(function($) {
	...
	var Notify = {
		...
		_build: function() {
        	...
        	if (o.enableClose) {
        	    var self = this;
        	    var btnCloseHtml = '<div class="place-right pointer"><a title="close notify" id="notify-close"><i class="fa fa-minus-square-o"></i><span></span></a></div>';
		        var $btnCloseHtml = $(btnCloseHtml);
		        $btnCloseHtml.on('click', function () {
		            self.close();
		        });
		        $btnCloseHtml.appendTo(this._notify);
		    }
            //end costome
			// add title
